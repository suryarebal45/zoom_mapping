-- models/silver/sv_feature_usage.sql
...
WITH bronze_feature_usage AS (
    SELECT * 
    FROM {{ ref('bz_feature_usage') }}  -- Changed from source() to ref()
),
...
feature_usage_with_dq AS (
    SELECT 
        *,
        -- Data Quality Score Calculation (macro)
        {{ calculate_dq_score('sv_feature_usage', 'usage_id') }} AS data_quality_score,

        -- Record Status based on DQ rules
        CASE 
            ...
            WHEN usage_count > 10000 THEN 'WARNING'  -- Outlier usage
            ELSE 'VALID'
